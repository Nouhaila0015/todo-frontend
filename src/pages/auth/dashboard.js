...
export default function Dashboard() {
    ...
    const [newTask, setNewTask] = useState({ titre: '', description: '', deadline: '', id_Status: 1 });
  ...
  useEffect(() => {
    const verifySession = async () => {
      try {
        ...
        fetchProjects();
        fetchTodayTasks(); // Fetch today's tasks when user session is verified

      } catch (error) {
        console.error('Auth error:', error);
        window.location.href = '/auth/login'; // Redirect to login if the session is invalid
      }
    };
    verifySession();
  }, []);
  ...
  const handleAddProject = async (e) => {
    e.preventDefault();
    if (!newProject.titre) {
        console.error("Titre du projet est manquant.");
        return;
    }

    try {
        // Ensure both titre and created_by are passed
        await axios.post(
            `http://localhost:3001/projets`,
            {
                titre: newProject.titre,
                created_by: user.id, // Ensure `user` is defined and has `id`
            },
            { withCredentials: true }
        );
        setNewProject({ titre: "" });
  ...
  const updateProject = async (projectId, newTitle) => {
    ...
    try {
        ...
        if (selectedProject?.id_projet === projectId) {
            setSelectedProject(null); // Reset selected project if it's the one being deleted
            setTasks([]); // Clear tasks related to the deleted project
        }
    } catch (error) {
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      <div className="w-72 bg-gray-100 border-r">
        ...
        {todayTasks.map((task) => (
  <div key={task.id_Tache} className="flex items-center gap-3 p-3 bg-white rounded-lg shadow mb-2">
    <input
      type="checkbox"
